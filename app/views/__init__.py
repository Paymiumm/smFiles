	...
	def post(self):
		if( 'user' in session):
			...
			if exec_account_update==True:
				...
				#code exception occured
				return json_response(res='false',reason="An exception was thrown, either SQL error or user does not exist")
		else:
...
param = api.parser()  # parsing arguments
param.add_argument('usr', type=inputs.regex("[A-z]{2,}"), help='Invalid username or email', required=True)
param.add_argument('t_y_pE', type=inputs.regex("[A-z]{2,}"), help='Specify the way you want to recieve your OTP', required=True)
...
param = api.parser()  # parsing arguments
param.add_argument('usr', type=inputs.regex("[A-z]{2,}"), help='Invalid username or email', required=True)
param.add_argument('pwd', type=inputs.regex("(\S+)"), help='Invalid OTP Format', required=True)
param.add_argument('t__Ukn__r_z_A_R', type=inputs.regex("(\S+)"), help='Invalid Device Identity Format', required=True)
